...
public final class LoggerTest {
    ...
    private static void levelsFilterMessages() {
        ...
        check(
                !output.contains("debug-hidden") && !output.contains("info-hidden"),
        ...
        check(
                output.contains("[DEBUG]") && output.contains("debug-visible"),
    ...
    private static void threadKindIsLogged() {
        ...
        check(
                platformOutput.contains("(p) platform-message"),
        ...
        String virtualOutput =
                capture(
                        () -> {
                            Thread virtual =
                                    Thread.ofVirtual().start(() -> Logger.info("virtual-message"));
                            join(virtual);
                        });
        check(
                virtualOutput.contains("(v) virtual-message"),
    ...
    private static void nullArgumentsAreRejected() {
        ...
        expectNullPointer(
                () ->
                        Logger.withLevel(
                                Logger.LogLevel.INFO, (java.util.function.Supplier<String>) null));
    }
